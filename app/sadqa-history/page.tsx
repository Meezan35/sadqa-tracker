...
// Mock user context hook for demo
const useUser = () => ({ userName: 'Demo User' });
...
export default function SadqaHistoryPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (storedData) {
        setSadqaData(JSON.parse(storedData));
      }
    } catch (error) {
  ...
  // Apply filters whenever the sadqaData or filter states change
  useEffect(() => {
